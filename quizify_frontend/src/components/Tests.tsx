    ...
    <div className="container mx-auto py-8">
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"> {
          quizzes.filter(
            (test) => new Date(test.time + test.duration) > new Date()
          )
          .map(quiz => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"> {
          quizzes.filter(
            (test) => new Date(test.time) < new Date() && new Date(test.time + test.duration) > new Date() 
          )
          .map(quiz => (
            <div key={quiz.id} className="bg-white rounded-lg shadow-md p-4">
              <h3 className="text-lg font-semibold mb-2">{quiz.title}</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"> {
          quizzes.filter(
            (test) => new Date(test.time) < new Date()
          )
          .map(quiz => (
            <div key={quiz.id} className="bg-white rounded-lg shadow-md p-4">
              <h3 className="text-lg font-semibold mb-2">{quiz.title}</h3>
